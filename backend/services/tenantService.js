...
const TenantCounter = require(
  "../models/tenantCounterModel"
);
...
/*
 * Added only for automatic tenant
 * header/business branding creation.
 */
...
const setTenantUsersActiveState =
  async (
    tenantId,
    isActive
  ) => {
 ...
 */
const getHighestExistingTenantSequence =
  async () => {
    const tenants =
      await Tenant.find({
        tenantCode: {
 ...
 *
 * If SRESTE tenant IDs already exist,
 * the counter starts from the highest
 * existing sequence so that old IDs
 * are not duplicated.
 */
const initializeTenantCounter =
  async () => {
    const existingCounter =
      await TenantCounter.findOne({
        key:
          TENANT_COUNTER_KEY,
      })
        .select("_id")
    ...
    try {
      ...
      /*
       * Another request may create the
       * same counter at the same time.
       *
       * Duplicate key means the counter
       * now exists, so generation can
       * safely continue.
       */
      if (
        error?.code !==
...
const ensureUniqueTenantFields =
  async ({
    slug,
    tenantCode,
    ownerEmail,
...
const throwFriendlyDatabaseError =
  (error) => {
    if (
      error?.code !==
      11000
    ...
    try {
      ...
      tenant =
        await Tenant.create({
          ...
          branding: {
            /*
             * No logo is required when a tenant is created.
             *
             * Until the Tenant Admin uploads a logo from
             * Header Management, the Admin Sidebar can show
             * the text/initial fallback based on storeName.
             */
            logo:
              normalizeText(
                payload
      ...
      /* =================================================
         ENSURE TENANT HEADER SETTINGS
      ================================================= */

      /*
       * IMPORTANT:
       *
       * Header Management is the single source of truth
       * for the tenant's storefront/admin sidebar logo.
       *
       * When a tenant is first created:
       *
       * businessName = tenant Store Name
       * logo         = ""
       * mobileLogo   = ""
       *
       * Therefore Tenant Admin initially sees a text/initial
       * logo based on Store Name.
       *
       * Later:
       *
       * Header Management
       ...
       * HeaderSetting.logo
       *      ↓
       * Storefront Header + Admin Sidebar
       *
       * findOneAndUpdate + upsert is intentionally used
       * because copyMasterTemplateToTenant may already create
       * a HeaderSetting. We do not create a duplicate.
       */

      provisioningStage =
        "ensure_header_setting";
      ...
      await HeaderSetting.findOneAndUpdate(
...

        {
          $set: {
            ...
            /*
             * New tenant starts without an uploaded logo.
             * Sidebar uses Store Name text/initial fallback.
             */
            logo: "",
      ...
      if (
        ...
        /*
         * HeaderSetting may have been created by either
         * master-template provisioning or the explicit
         * ensure_header_setting step.
         *
         * Delete it explicitly during rollback so no
         * orphan tenant branding remains.
         */
        await HeaderSetting.deleteMany(
          ...
          (
            ...
            console.error(
              "Tenant header setting rollback error:",
              rollbackError
            );
          }
        );
        ...
        let message =
          ...
          "copy_master_template"
            ...
            : provisioningStage ===
              ...
              : provisioningStage ===
                  "ensure_header_setting"
                ? "Tenant header settings could not be provisioned"
...
const getTenants =
  async (
    ...
    if (
      queryParams.status &&
      ALLOWED_TENANT_STATUSES.includes(
        queryParams.status
      )
    ) {
      query.status =
...
const updateTenant =
  async (
    ...
    /*
     * Tenant ID is permanent.
     * Even if a client sends tenantCode,
    ...
    try {
      ...
      return throwFriendlyDatabaseError(
        error
      );
    }
  };
